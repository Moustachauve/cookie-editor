(function () {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    function filterCookies(filterText) {
      console.log("filtering cookie list...");
      console.log("filtering by: ", filterExpression);
      console.error("cookies: ", loadedCookies);
      Array.from(
        document.getElementById("cookie-container").children[0].children
    ...
    function saveCookie(
      ...
      if (oldName !== name || oldHostOnly !== hostOnly) {
        cookieHandler.removeCookie(oldName, getCurrentTabUrl(), function () {
          cookieHandler.saveCookie(cookie, getCurrentTabUrl(), function (
            error,
            cookie
          ) {
            if (error) {
    ...
    document
      ...
      .addEventListener("keyup", (e) => filterCookies(e.target.value));
  ...
  function showNotification() {
    ...
    notificationElement.querySelector(
      "span"
    ).textContent = notificationQueue.shift();
